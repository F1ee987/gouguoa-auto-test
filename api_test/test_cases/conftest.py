...
# ---------- 私有辅助函数 ----------
def _login_session(csv_row_index: int) -> RequestHandle:
    """
    ...
    assert login_res.json().get('msg') == "登录成功", f"登录失败"
    print("✔连接成功")
    return sess
...
def normal_api_login() -> Generator[RequestHandle, None, None]:
    """
    使用普通员工用户登录（CSV 第5行，索引4）
    """
    sess = _login_session(csv_row_index=4)   # 普通员工所在行
    yield sess
    print("✔关闭连接...")
    sess.close()
...
@pytest.fixture(scope='function')
def admin_api_login() -> Generator[RequestHandle, None, None]:
    """
    使用管理员用户登录（CSV 第2行，索引1）
    """
    sess = _login_session(csv_row_index=1)   # 管理员所在行
    yield sess
    print("✔关闭连接...")
    sess.close()